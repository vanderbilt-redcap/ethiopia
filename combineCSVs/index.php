...
if (isset($_GET['submit']) && (isset($_POST['project_id'])) && ($_POST['project_id'] != '')) {
    ...
    $sql = "SELECT app_title, project_id FROM redcap_projects WHERE app_title LIKE 'ImPACT Africa Global Perioperative Outcomes%' OR app_title LIKE 'ImPACT Ethiopia GPO%' ORDER BY project_id";
    $q = db_query($sql);
    if ($error = db_error()) {
        echo "ERROR: $error<br>";
    }
